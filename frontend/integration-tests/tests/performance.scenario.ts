...
describe('Performance test', () => {
      ...
      (performance.getEntriesByType('resource') as any)
        .filter(
          ({ name }) =>
            name.endsWith('.js') &&
            name.indexOf('main') > -1 &&
            name.indexOf('runtime') === -1
        )
        .map(({ name, decodedBodySize }) => ({
          name: name.split('/').slice(-1)[0],
          size: Math.floor(decodedBodySize / 1024)
        }))
        .reduce(
          (acc, val) =>
            acc.concat(`${val.name.split('-')[0]}: ${val.size} KB, `),
          ''
        )
    );

    writeFileSync(
      path.resolve(__dirname, '../../gui_test_screenshots/bundle-analysis.txt'),
      resources as any
    );
  ...
  it('downloads new bundle for "Overview" route', async () => {
    ...
    const overviewChunk = await browser.executeScript<any>(() =>
      performance
        .getEntriesByType('resource')
        .find(
          ({ name }) =>
            name.endsWith('.js') && name.indexOf('cluster-overview') > -1
        )
    );
  ...
  chunkedRoutes.forEach((route, routeName) => {
    ...
    const routeChunkFor = function() {
      ...
      return performance
        .getEntriesByType('resource')
        .find(
          ({ name }) => name.endsWith('.js') && name.indexOf(chunkName) > -1
        );
    };
    ...
    it(`downloads new bundle for ${routeName}`, async () => {
      ...
      const routeChunk = await browser.executeScript<any>(
        routeChunkFor,
        routeName
      );
