...
import { referenceForModel, MachineKind, NodeKind, MachineHealthCheckKind, MachineHealthCondition } from '@console/internal/module/k8s';
import { useK8sWatchResource, WatchK8sResult, WatchK8sResource } from '@console/internal/components/utils/k8s-watch-hook';
...
export const HealthChecksPopup: React.FC<HealthChecksPopupProps> = ({ conditions = [], machineHealthChecks }) => {
  ...
  const grouppedConditions = Object.values(
    ...
    if (failing) {
      ...
      reboot = machineHealthChecks?.[0]?.metadata?.annotations?.['machine.openshift.io/remediation-strategy'] === 'external-baremetal';
    }
    return {
      title: cds[0].type,
      value: failing ? 'Failing' : 'Okay',
      icon: failing ? healthStateMapping[HealthState.WARNING].icon : healthStateMapping[HealthState.OK].icon,
    };
  });
  return (
    <>
      ...
      {!!machineHealthChecks?.length && (
        <StatusPopupSection firstColumn={pluralize(machineHealthChecks.length, MachineHealthCheckModel.label, MachineHealthCheckModel.labelPlural, false)}>
          {machineHealthChecks.map(({ metadata }) => (
            <Status key={metadata.uid}>
              <ResourceLink kind={referenceForModel(MachineHealthCheckModel)} name={metadata.name} namespace={metadata.namespace} className="co-status-popup__title" />
            </Status>
          ))}
        </StatusPopupSection>
      )}
      ...
      {conditionFailing && (
        <Alert variant="warning" isInline title={`${reboot ? 'Reboot' : 'Reprovision'} pending`} className="co-node-health__popup-alert">
          {CONDITIONS_WARNING(reboot)}
        </Alert>
      )}
      {machineHealthChecks?.length > 1 && (
        <Alert variant="warning" isInline title="Multiple resources" className="co-node-health__popup-alert">
...
const isConditionFailing = (node: NodeKind, { type, status, timeout }: MachineHealthCondition): boolean => {
...
export const getMachineHealth = (node: NodeKind, machine: WatchK8sResult<MachineKind>, healthChecks: WatchK8sResult<MachineHealthCheckKind[]>): MachineHealth => {
  ...
  return {
    ...
    details: matchingHC.length > 1 ? 'Multiple resources' : failingConditions ? `${pluralize(failingConditions, 'condition')} failing` : `${pluralize(conditions.length, 'condition')} passing`,
    conditions,
    matchingHC,
  };
};
...
export const HealthChecksItem: React.FC = () => {
  ...
  _.forEach(healthState.conditions, c => {
    if (c.failing) {
      ...
      reboot = healthState.matchingHC?.[0]?.metadata?.annotations?.['machine.openshift.io/remediation-strategy'] === 'external-baremetal';
      return false;
    }
    return true;
  });
  ...
  return (
    <HealthItem title="Health Checks" popupTitle="Health Checks" {...healthState}>
      <HealthChecksPopup conditions={healthState.conditions} machineHealthChecks={healthState.matchingHC} />
    </HealthItem>
  );
};
