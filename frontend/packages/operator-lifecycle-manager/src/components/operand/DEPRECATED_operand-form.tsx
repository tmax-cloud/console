...
import { GroupVersionKind, ImagePullPolicy, k8sCreate, K8sResourceKind, kindForReference, modelFor, NodeAffinity as NodeAffinityType } from '@console/internal/module/k8s';
...
const hasDescriptor = (field: OperandField, prefix: string, suffix: string = null): boolean => {
  return suffix ? _.includes(field.capabilities, `${prefix}${suffix}`) : _.some(field.capabilities, capability => capability.startsWith(prefix));
};
 ...
 * only.
 */
const parseGroupDescriptor = (field: OperandField): { groupName?: string; groupType?: string; regexMatch?: string } => {
  const groupDescriptor = _.find(field.capabilities, descriptor => descriptor.startsWith(SpecCapability.fieldGroup) || descriptor.startsWith(SpecCapability.arrayFieldGroup));
 ...
 */
const parseArrayPath = (path: string): { index?: number; regexMatch?: string; pathBeforeIndex?: string; pathAfterIndex?: string } => {
  const [regexMatch, pathBeforeIndex, index, pathAfterIndex] = path.match(ARRAY_INDEX_PATTERN) || [];
  return regexMatch ? { index: _.parseInt(index), regexMatch, pathBeforeIndex, pathAfterIndex } : { regexMatch };
};
 ...
 */
const modifyArrayFieldPathIndex = (path: string, operation: (index?: number) => string | number): string => {
  const { regexMatch, index, pathBeforeIndex, pathAfterIndex } = parseArrayPath(path);
  return !regexMatch ? path : `${pathBeforeIndex}[${operation(index)}]${pathAfterIndex && `.${pathAfterIndex}`}`;
};
...
const defaultValueFor = (capabilities: Capability[]): any => {
  // String fields
  if (_.intersection(capabilities, [SpecCapability.podCount, SpecCapability.password, SpecCapability.text, SpecCapability.number, SpecCapability.select]).length > 0) {
    return '';
  }
  ...
  if (capabilities.includes(SpecCapability.podAffinity) || capabilities.includes(SpecCapability.podAntiAffinity)) {
...
const capabilitiesFor = (property: JSONSchema7): SpecCapability[] => {
  if (property.enum) {
    return _.map(property.enum || [], (option: string) => `${SpecCapability.select}${option}` as SpecCapability);
  }
 ...
 */
const flattenNestedProperties = (property: JSONSchema7, name: string, providedAPI: ProvidedAPI, obj: K8sResourceKind, { currentCapabilities = [], currentPath = [], fields = [], required = false }: FlattenNestedPropertiesAccumulator): OperandField[] => {
  // Null check
  if (!property) {
    return fields;
  }
  ...
  const handleObjectProperty = (): OperandField[] =>
    _.flatMap(property.properties, (nestedProperty, nestedPropertyName) =>
      flattenNestedProperties(nestedProperty as JSONSchema7, nestedPropertyName, providedAPI, obj, {
        currentCapabilities: [...currentCapabilities, `${SpecCapability.fieldGroup}${name}` as SpecCapability],
  ...
  const handleArrayProperty = (): OperandField[] => {
    ...
    return _.flatMapDepth(
      (property.items as JSONSchema7)?.properties ?? {},
      (nestedProperty, nestedPropertyName) =>
        ...
        _.times(n, index =>
          flattenNestedProperties(nestedProperty as JSONSchema7, nestedPropertyName, providedAPI, obj, {
            currentCapabilities: [...currentCapabilities, `${SpecCapability.arrayFieldGroup}${name}` as SpecCapability],
  ...
  const handleAtomicProperty = (): OperandField[] => {
    ...
    return [
      ...fields,
      {
        capabilities: _.union(providedAPIField?.['x-descriptors'] || [], currentCapabilities, capabilitiesFor(property as JSONSchema7)),
...
const getPropertyDepth = (property: JSONSchema7, depth: number = 0): number => {
  ...
  return Math.max(0, ..._.map(property?.properties || (property?.items as JSONSchema7)?.properties, nestedProperty => getPropertyDepth(nestedProperty as JSONSchema7, depth + 1)));
};
 ...
 */
const fieldsForOpenAPI = (schema: JSONSchema7, providedAPI: ProvidedAPI, obj: K8sResourceKind, depth: number = MAX_DEPTH): OperandField[] => {
  return _.reduce(
    schema?.properties || {},
    (openAPIFieldsAccumulator: OperandField[], property: JSONSchema7, propertyName: string) => {
 ...
 */
const specDescriptorToFields = ({ description, displayName, path, 'x-descriptors': capabilities = [] }: Descriptor, obj: K8sResourceKind): OperandField[] => {
...
export const DEPRECATED_CreateOperandForm: React.FC<OperandFormProps> = ({ formData, csv, schema, model, onChange, providedAPI, match, next }) => {
  ...
  const [fields, setFields] = React.useState<OperandField[]>(() => {
    ...
    const schemaFields = fieldsForOpenAPI(schema?.properties?.spec as JSONSchema7, providedAPI, formData);
    ...
    const descriptorFields = _.reduce(
      ...
      (providedAPIFieldsAccumulator, specDescriptor) => {
        ...
        return [...providedAPIFieldsAccumulator, ...specDescriptorToFields(specDescriptor, formData)];
      },
      [],
    );
  ...
  const [advancedFields = [], arrayFields = [], groupFields = [], normalFields = []]: OperandField[][] = React.useMemo(
    (): OperandField[][] =>
      _.reduce(
        fields,
        ([advancedFieldsAccumulator = [], arrayFieldsAccumulator = [], groupFieldsAccumulator = [], normalFieldsAccumulator = []], field) => {
          if (hasDescriptor(field, SpecCapability.arrayFieldGroup)) {
            return [advancedFieldsAccumulator, [...arrayFieldsAccumulator, field], groupFieldsAccumulator, normalFieldsAccumulator];
          }
          if (hasDescriptor(field, SpecCapability.fieldGroup)) {
            return [advancedFieldsAccumulator, arrayFieldsAccumulator, [...groupFieldsAccumulator, field], normalFieldsAccumulator];
          }
          if (hasDescriptor(field, SpecCapability.advanced)) {
            return [[...advancedFieldsAccumulator, field], arrayFieldsAccumulator, groupFieldsAccumulator, normalFieldsAccumulator];
          }
          return [advancedFieldsAccumulator, arrayFieldsAccumulator, groupFieldsAccumulator, [...normalFieldsAccumulator, field]];
        },
        [],
      ),
    [fields],
  ...
  const submit = event => {
        ...
        const fieldErrors = _.map(field.validation, (val, rule: Validations) => {
          ...
          switch (rule) {
            ...
            case Validations.pattern:
              return new RegExp(val as string).test(formVal) ? null : `Does not match required pattern ${val}`;
            default:
              return null;
          }
        });
    ...
    if (_.isEmpty(_.compact(_.values(errors)))) {
      k8sCreate(model, model.namespaced ? immutableFormData.setIn(['metadata', 'namespace'], match.params.ns).toJS() : immutableFormData.toJS())
  ...
  const inputFor = ({ capabilities, displayName, path, required, validation }: OperandField) => {
    ...
    if (capabilities.find(c => c.startsWith(SpecCapability.fieldDependency))) {
      ...
      const controlFieldPathList = _.uniq(controlFieldInfoList.map(c => c.split(SpecCapability.fieldDependency)[1]).reduce((infoList, info) => [info.split(':')[0], ...infoList], []));
      const controlFieldPath = _.isArray(controlFieldPathList) && controlFieldPathList.length === 1 ? controlFieldPathList[0] : null;
      const currentControlFieldValue = !_.isNil(getFormData(`spec.${controlFieldPath}`)) ? getFormData(`spec.${controlFieldPath}`).toString() : null;
      const expectedControlFieldValueList = controlFieldInfoList.map(c => c.split(SpecCapability.fieldDependency)[1]).reduce((infoList, info) => [info.split(':')[1], ...infoList], []);
    ...
    if (capabilities.includes(SpecCapability.podCount)) {
      return <NumberSpinner id={id} className="pf-c-form-control" value={currentValue} onChange={({ currentTarget: { value } }) => handleFormDataUpdate(path, _.toInteger(value))} changeValueBy={operation => handleFormDataUpdate(path, _.toInteger(currentValue) + operation)} autoFocus required />;
    }
    if (capabilities.includes(SpecCapability.resourceRequirements)) {
    ...
    if (capabilities.includes(SpecCapability.password)) {
      return (
        <div>
          <input className="pf-c-form-control" id={id} type="password" {...validation} onChange={({ currentTarget: { value } }) => handleFormDataUpdate(path, value)} value={currentValue} />
        </div>
      );
    }
    ...
    if (capabilities.includes(SpecCapability.checkbox)) {
      return <Checkbox id={id} style={{ marginLeft: '10px' }} isChecked={(_.isNil(currentValue) ? false : currentValue) as boolean} label={displayName} required={required} onChange={value => handleFormDataUpdate(path, value)} />;
    }
    if (capabilities.includes(SpecCapability.booleanSwitch)) {
      return <Switch key={id} id={id} isChecked={(_.isNil(currentValue) ? false : currentValue) as boolean} onChange={value => handleFormDataUpdate(path, value)} label="True" labelOff="False" />;
    }
    if (capabilities.includes(SpecCapability.imagePullPolicy)) {
      return (
        <RadioGroup
    ...
    if (capabilities.includes(SpecCapability.updateStrategy)) {
      ...
      return <ConfigureUpdateStrategy strategyType={currentValue.get('type')} maxUnavailable={currentValue.getIn(_.toPath(maxUnavailablePath))} maxSurge={currentValue.getIn(_.toPath(maxSurgePath))} onChangeStrategyType={value => handleFormDataUpdate(`${path}.type`, value)} onChangeMaxUnavailable={value => handleFormDataUpdate(`${path}.${maxUnavailablePath}`, value)} onChangeMaxSurge={value => handleFormDataUpdate(`${path}.${maxSurgePath}`, value)} replicas={1} uid={id} />;
    }
    if (capabilities.includes(SpecCapability.text)) {
      return (
        <div>
          <input key={id} className="pf-c-form-control" id={id} type="text" onChange={({ currentTarget: { value } }) => handleFormDataUpdate(path, value)} value={currentValue} />
        </div>
      );
    }
    if (capabilities.includes(SpecCapability.number)) {
      return (
        <div>
          <input key={path} className="pf-c-form-control" id={id} type="number" onChange={({ currentTarget: { value } }) => handleFormDataUpdate(path, value !== '' ? _.toNumber(value) : '')} value={currentValue !== '' ? _.toNumber(currentValue) : ''} />
        </div>
      );
    }
    if (capabilities.includes(SpecCapability.nodeAffinity)) {
      return (
        <div style={{ marginLeft: '15px' }}>
          <NodeAffinity affinity={currentValue.toJS() as NodeAffinityType} onChange={value => handleFormDataUpdate(path, Immutable.fromJS(value))} uid={id} />
        </div>
      );
    }
    if (capabilities.includes(SpecCapability.podAffinity) || capabilities.includes(SpecCapability.podAntiAffinity)) {
      return (
        <div style={{ marginLeft: '15px' }}>
          <PodAffinity affinity={currentValue.toJS()} onChange={value => handleFormDataUpdate(path, Immutable.fromJS(value))} uid={id} />
        </div>
      );
    }
  ...
  const removeArrayFieldGroup = (fieldLists: OperandField[][], indexToRemove: number) => {
    ...
    const [regexMatch, formDataPathToRemove] = (fieldPathsToRemove?.[0] || '').match(/^(.*\[\d+\]).*$/) || [];
            ...
            <React.Fragment key={`${groupName}-${index}`}>
              ...
              {fieldLists.length > 1 && (
                <div className="row co-array-field-group__remove">
                  <Button type="button" className="co-array-field-group__remove-btn" onClick={() => removeArrayFieldGroup(fieldLists, index)} variant="link">
              ...
              {_.map(fieldList, field => (
                <OperandFormInputGroup key={field.path} error={formErrors?.[field.path]} field={field} input={inputFor(field)} />
              ))}
            </React.Fragment>
          ))}
          <div className="row">
  ...
  const renderNormalFields = () => _.map(normalFields, field => <OperandFormInputGroup key={field.path} field={field} input={inputFor(field)} error={formErrors?.[field.path]} />);

  const renderAdvancedFields = () =>
    advancedFields.length > 0 && (
      <div>
        <ExpandCollapse textExpanded="Advanced Configuration" textCollapsed="Advanced Configuration">
          {_.map(advancedFields, field => (
            <OperandFormInputGroup key={field.path} field={field} input={inputFor(field)} error={formErrors?.[field.path]} />
          ))}
        </ExpandCollapse>
      </div>
    );
          ...
          <form className="co-dynamic-form" onSubmit={submit}>
            <Accordion asDefinitionList={false} className="co-dynamic-form__accordion">
              <div key={'metadata.name'} className="form-group">
                ...
                <input className="pf-c-form-control" type="text" onChange={({ target: { value } }) => handleFormDataUpdate('metadata.name', value)} value={immutableFormData.getIn(['metadata', 'name']) || 'example'} id="root_metadata_name" required />
              </div>
              <div key={'root_metadata_labels'} className="form-group">
                <label className="control-label" htmlFor="tags-input">
                  Labels
                </label>
                <SelectorInput onChange={value => handleFormDataUpdate('metadata.labels', Immutable.fromJS(SelectorInput.objectify(value)))} tags={labelTags} />
              </div>
