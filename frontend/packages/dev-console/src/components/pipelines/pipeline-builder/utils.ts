...
export const getErrorMessage = (
  ...
  const hasRequestedError = errorList.filter(error =>
    errorTypes.includes(error)
  );
  return hasRequestedError.length > 0
    ? TASK_ERROR_STRINGS[hasRequestedError[0]]
    : null;
};

export const taskParamIsRequired = (
  param: PipelineResourceTaskParam
): boolean => {
...
export const convertResourceToTask = (
  ...
  return {
    ...
    taskRef: {
      kind:
        resource.kind === ClusterTaskModel.kind
          ? ClusterTaskModel.kind
          : undefined,
      name: resource.metadata.name
    },
  ...
  return {
    ...
    spec: {
      ...
      tasks: tasks.map(task =>
        removeEmptyDefaultParams(removeListRunAfters(task, listIds))
      )
    }
  };
};
