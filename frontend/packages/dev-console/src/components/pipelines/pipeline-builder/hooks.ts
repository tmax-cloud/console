...
export const useTasks = (namespace?: string): UseTasks => {
  const [namespacedTasks, setNamespacedTasks] = React.useState<
    PipelineResourceTask[]
  >(null);
  const [clusterTasks, setClusterTasks] = React.useState<
    PipelineResourceTask[]
  >(null);
    ...
    if (!namespacedTasks) {
      if (!namespace) {
        ...
        k8sList(TaskModel, { ns: namespace })
          ...
          .catch(() => {
            setLoadErrorMsg(
              `Failed to load namespace Tasks. ${loadErrorMsg || ''}`
            );
          });
      }
    }
...
export const useResourceValidation = (
  ...
  React.useEffect(() => {
    ...
    if (!_.isEmpty(errors) || previousErrorIds.length > 0) {
      ...
      const currentErrorIds = Object.keys(outputErrors).filter(
        id => !!outputErrors[id]
      );
