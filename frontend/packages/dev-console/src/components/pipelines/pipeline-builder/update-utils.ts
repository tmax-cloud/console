...
const getErrors = (
  ...
  const missingOutputResources =
    requiredOutputResources - taskOutputResources > 0;
...
const applyResourceUpdate = (
  ...
  const {
    resourceTarget,
    selectedPipelineResource,
    taskResourceName
  } = resources;

  const existingResources: PipelineTaskResource[] =
    pipelineTask.resources?.[resourceTarget] || [];
  const filteredResources = existingResources.filter(
    (resource: PipelineTaskResource) => {
  ...
  switch (type) {
    ...
    case UpdateOperationType.REMOVE_TASK:
      return removeTask(
        tasks,
        listTasks,
        data as UpdateOperationRemoveTaskData
      );
    case UpdateOperationType.UPDATE_TASK:
      return updateTask(
        tasks,
        listTasks,
        data as UpdateOperationUpdateTaskData
      );
