...
import { coFetchText } from '../../../../../../public/co-fetch';
import { errorModal } from '../../../../../../public/components/modals';
...
export const getDownloadAllLogsCallback = (
  ...
  const fetchLogs = async (tasksPromise: Promise<StepsWatchUrl>) => {
    ...
    const allRequests: Promise<string>[] = sortedTaskRuns.reduce(
      (acc, currTask) => {
        const task = tasks[currTask];
        const promises: Promise<string>[] = Object.keys(task.steps).map(
          (step, i) => {
            let heading = '';
            if (i === 0) {
              heading += `${task.name}\n\n`;
