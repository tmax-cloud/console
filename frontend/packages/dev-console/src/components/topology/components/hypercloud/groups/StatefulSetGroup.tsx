...
const StatefulSetGroup: React.FC<StatefulSetGroupProps> = ({ element, selected, onSelect, dndDropRef, droppable, canDrop, dropTarget, onContextMenu, contextMenuOpen, dragging }) => {
  ...
  return (
    ...
    >
      ...
      <Layer id="groups">
        ...
        >
          <path ref={dndDropRef} className="odc-statefulset-group__bg" filter={createSvgIdUrl(hover || labelHover || dragging || contextMenuOpen || dropTarget ? NODE_SHADOW_FILTER_ID_HOVER : NODE_SHADOW_FILTER_ID)} d={pathRef.current} />
        </g>
      </Layer>
      {showLabels && (
        <SvgBoxedText className="odc-statefulset-group__label" kind="statefulset" x={labelLocation.current[0]} y={labelLocation.current[1] + hullPadding(labelLocation.current) + 24} paddingX={8} paddingY={5} dragRef={dragLabelRef}>
          {element.getLabel()}
        </SvgBoxedText>
      )}
    </g>
...
export default observer(StatefulSetGroup);
