...
const DaemonSetNode: React.FC<DaemonSetNodeProps> = ({ element, selected, onSelect, dndDropRef, canDrop, dropTarget, onContextMenu, contextMenuOpen, dragging }) => {
  ...
  return (
    ...
    >
      <NodeShadows />
      <rect ref={dndDropRef} filter={createSvgIdUrl(hover || dragging || contextMenuOpen || dropTarget ? NODE_SHADOW_FILTER_ID_HOVER : NODE_SHADOW_FILTER_ID)} className="odc-daemonset-group__bg" x={0} y={0} width={width} height={height} rx="5" ry="5" />
      <GroupNode element={element} kind={DaemonSetModel.kind} groupResources={element.getData().groupResources} />
    </g>
  );
};

export default observer(DaemonSetNode);
