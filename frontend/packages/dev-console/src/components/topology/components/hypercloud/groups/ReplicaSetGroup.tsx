...
const ReplicaSetGroup: React.FC<ReplicaSetGroupProps> = ({ element, selected, onSelect, dndDropRef, droppable, canDrop, dropTarget, onContextMenu, contextMenuOpen, dragging }) => {
  ...
  return (
    ...
    >
      ...
      {showLabels && (
        <SvgBoxedText className="odc-replicaset-group__label" kind="replicaset" x={labelLocation.current[0]} y={labelLocation.current[1] + hullPadding(labelLocation.current) + 24} paddingX={8} paddingY={5} dragRef={dragLabelRef}>
          {element.getLabel()}
        </SvgBoxedText>
      )}
    </g>
...
export default observer(ReplicaSetGroup);
