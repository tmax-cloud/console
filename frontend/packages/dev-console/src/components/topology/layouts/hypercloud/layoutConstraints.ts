...
import { TYPE_CONNECTS_TO, TYPE_REPLICASET_GROUP, TYPE_WORKLOAD } from '../../components/const';
...
export const layoutConstraints = (nodes: ColaNode[], groups: ColaGroup[], edges: ColaLink[], options: LayoutOptions): any[] => {
  ...
  [...groups, ...nodes]
    .filter(g => g.element.getType() === TYPE_WORKLOAD || (g.element.getType() === TYPE_REPLICASET_GROUP && g.element.isCollapsed() === true))
    .forEach(g => {
