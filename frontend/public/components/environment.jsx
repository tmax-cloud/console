...
export const EnvironmentPage = connect(stateToProps)(
  class EnvironmentPage extends PromiseComponent {
    ...
    _envVarsToNameVal(finalEnvPairs) {
      return _.filter(finalEnvPairs, finalEnvPair => !_.isEmpty(finalEnvPair[NameValueEditorPair.Name])).map(finalPairForContainer => {
    ...
    render() {
      ...
      const { rawEnvData, readOnly, obj, t } = this.props;
      ...
      const containerVars = currentEnvVars.map((envVar, i) => {
        ...
        return (
          <div key={keyString} className="co-m-pane__body-group">
            {_.isArray(rawEnvData) && (
              <h2 className="co-section-heading co-section-heading--contains-resource-icon">
                <ResourceIcon kind="Container" className="co-m-resource-icon--align-left co-m-resource-icon--flex-child" /> {keyString}
              </h2>
            )}
            <NameValueEditorComponent nameValueId={i} nameValuePairs={envVar} updateParentData={this.updateEnvVars} addString={t('CONTENT:ADDVALUE')} valueString={t('CONTENT:VALUE')} nameString={t('CONTENT:NAME')} readOnly={readOnly} allowSorting={true} configMaps={configMaps} secrets={secrets} />
          </div>
        );
      });
            ...
            <div className="environment-buttons">
              ...
              {success && (
                <p className="alert alert-success">
                  <span className="pficon pficon-ok" aria-hidden="true"></span>
                  {success}
                </p>
              )}
              {!readOnly && (
