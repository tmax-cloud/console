...
const DomainRow = () =>
  ...
  function DomainRow({ obj }) {
    return (
      <div className="row co-resource-list__item">
        <div className="col-xs-4 col-sm-4 co-resource-link-wrapper">
          {!HDCModeFlag && <ResourceCog actions={menuActions} kind="Domain" resource={obj} />}
          <ResourceLink kind="Domain" name={obj.metadata.name} namespace={obj.metadata.namespace} title={obj.metadata.name} />
        </div>
    ...
    return (
      <React.Fragment>
        <div className="co-m-pane__body">
          <SectionHeading text={t('ADDITIONAL:OVERVIEWTITLE', { something: ResourcePlural('Domain', t) })} />
...
export const DomainList = props => {
  const { kinds } = props;
  const Row = DomainRow(kinds[0]);
  Row.displayName = 'DomainRow';
  return <List {...props} Header={DomainHeader} Row={Row} />;
};
DomainList.displayName = DomainList;

export const DomainsPage = props => {
  const { t } = useTranslation();
  return HDCModeFlag ? <ListPage {...props} ListComponent={DomainList} canCreate={false} kind="Domain" /> : <ListPage {...props} ListComponent={DomainList} createButtonText={t('ADDITIONAL:CREATEBUTTON', { something: ResourcePlural(props.kind, t) })} canCreate={true} kind="Domain" />;
};
DomainsPage.displayName = 'DomainsPage';

export const DomainsDetailsPage = props => {
  const { t } = useTranslation();
  ...
  return <DetailsPage {...props} kind="Domain" menuActions={menu} pages={page} />;
};

DomainsDetailsPage.displayName = 'DomainsDetailsPage';
