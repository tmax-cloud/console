...
const StatusIconRow = ({ state, text }) => {
  ...
  return (
    <div className={classNames('co-m-status', [`co-m-status--${state}`])}>
  ...
  return (
    <div className="row">
      <div className="col-xs-12">
        <h4 className="cluster-overview-cell__title">{header}</h4>
        {children}
      </div>
    </div>
  );
...
const SoftwareDetailRow = ({ title, detail, text, children }) => {
  return (
    <div className="row cluster-overview-cell__info-row">
...
export const SoftwareDetails = connectToFlags(FLAGS.OPENSHIFT)(
  class SoftwareDetails extends SafetyFirst {
    ...
    render() {
      ...
      return (
        <React.Fragment>
          <SoftwareDetailRow title={t('CONTENT:KUBERNETES')} detail={kubernetesVersion} text="Kubernetes version could not be determined." />
