...
export const ServiceClassDetail = ({ detail }) => {
  const { name, imageUrl, description, providerDisplayName, longDescription } = detail;
  return (
    <div>
      ...
      <hr />
      <div>
        <p style={{ margin: '10px 0 0 0', fontSize: '13px', color: '#6A6A6A', textAlign: 'initial' }}>{longDescription}</p>
      </div>
    </div>
  );
};
