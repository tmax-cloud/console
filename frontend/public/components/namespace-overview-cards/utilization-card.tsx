...
import { OverviewQuery, namespaceUtilizationQueries, top25ConsumerQueries, multilineQueries } from '../dashboard/dashboards-page/cluster-dashboard/queries';
...
const getQueries = (itemExtensions: DashboardsOverviewUtilizationItem[], namespace: String) => {
  const pluginQueries = {};
  ...
  return _.defaults(pluginQueries, namespaceUtilizationQueries(namespace));
};

export const UtilizationCard = ({ namespace }) => {
  const { t } = useTranslation();
  const itemExtensions = useExtensions<DashboardsOverviewUtilizationItem>(isDashboardsOverviewUtilizationItem);

  const queries = React.useMemo(() => getQueries(itemExtensions, namespace), [itemExtensions]);
