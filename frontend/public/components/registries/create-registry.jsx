...
class RegistryFormComponent extends React.Component {
  ...
  save = e => {
    ...
    if (this.state.registry.spec.loginPassword !== this.state.loginPwdConfirm) {
      ...
      (this.props.isCreate
        ? k8sCreate(ko, newRegistry)
        : k8sUpdate(ko, newRegistry, metadata.namespace, newRegistry.metadata.name)
      ).then(() => {
        ...
        <fieldset disabled={!this.props.isCreate}>
          <Section label={t('CONTENT:NAME')} isRequired={true}>
            <input className="form-control"
              ...
              value={this.state.registry.metadata.name}
              id="registry-name"
              required />
          </Section>
          ...
          <Section label={t('CONTENT:REGISTRYCREATIONIMAGE')} isRequired={true}>
            <input className="form-control"
              placeholder={t('STRING:REGISTRY-CREATE_0')}
              type="text"
              onChange={this.onImageChanged}
              value={this.state.registry.spec.image}
              id="registry-image"
              required
            />
          ...
          <Section label={t('CONTENT:LOGININFORMATION')} isRequired={true}>
            <LabelInput
              ...
              value={this.state.registry.spec.loginId}
              id="registry-id"
            />
            <LabelInput
              label={t('CONTENT:PASSWORD')}
              onChange={this.onPwdChanged}
              value={this.state.registry.spec.loginPassword}
              id="registry-password"
              isPassword={true}
            />
            <LabelInput
              label={t('CONTENT:PASSWORDCONFIRM')}
              onChange={this.onPwdConfirmChanged}
              value={this.state.loginPwdConfirm}
          ...
          <Section label={t('CONTENT:SERVICE')} isRequired={true}>
            ...
            {this.state.serviceType === 'ingress' ? <LabelInput
              label={t('CONTENT:DOMAINNAME')}
              onChange={this.onServiceDomainNameChanged}
              value={this.state.domainName}
              id="registry-domain-name"
              placeholder="192.168.6.110.nip.io"
            /> : ''}
            <LabelInput
              label={t('CONTENT:PORT')}
              onChange={this.onServicePortChanged}
          ...
          <ButtonBar errorMessage={this.state.error} inProgress={this.state.inProgress} >
            ...
            <Link to={formatNamespacedRouteForResource('registries')} className="btn btn-default" id="cancel">{t('CONTENT:CANCEL')}</Link>
          </ButtonBar>
        </fieldset>
      </form>
    </div>;
...
export const CreateRegistry = ({ match: { params } }) => {
  ...
  return <RegistryFormComponent
    ...
    registryTypeAbstraction={params.type}
    titleVerb="Create"
    isCreate={true}
  />;
};
