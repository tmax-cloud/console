...
export const Resources = connectToPlural((props: ResourceProps) => {
  ...
  const thisDescription: CRDDescription[] = _.get(clusterServiceVersion, 'spec.customresourcedefinitions.owned', []).find(def => def.name.split('.')[0] === _.get(kindObj, 'path', ''));

  const crds = _.get(thisDescription, 'resources', [])
    .filter(ref => ref.name)
    .map(ref => ref.kind);
  const firehoseResources = _.get(
    thisDescription,
    'resources',
    ['Deployment', 'Service', 'ReplicaSet', 'Pod', 'Secret', 'ConfigMap'].map(kind => ({ kind })),
  ...
  const flattenFor = (parentObj: K8sResourceKind) => (resources: { [kind: string]: { data: K8sResourceKind[] } }) => {
    return _.flatMap(resources, (resource, kind: string) => resource.data.map(item => ({ ...item, kind }))).reduce((owned, resource) => {
      return (resource.metadata.ownerReferences || []).some(ref => ref.uid === parentObj.metadata.uid || owned.some(({ metadata }) => metadata.uid === ref.uid)) ? owned.concat([resource]) : owned;
    }, [] as K8sResourceKind[]);
  };
  ...
  const ResourceHeader: React.SFC<ResourceHeaderProps> = headerProps => (
    <ListHeader>
      <ColHead {...headerProps} className="col-xs-4" sortField="metadata.name">
        Name
      </ColHead>
      <ColHead {...headerProps} className="col-xs-2" sortField="kind">
        Type
      </ColHead>
      <ColHead {...headerProps} className="col-xs-2" sortField="status.phase">
        Status
      </ColHead>
      <ColHead {...headerProps} className="col-xs-4" sortField="metadata.creationTimestamp">
        Created
      </ColHead>
    </ListHeader>
  );

  const ResourceRow: React.SFC<ResourceRowProps> = ({ obj }) => {
    const { t } = useTranslation();
    return (
      <div className="row co-resource-list__item">
        <div className="col-xs-4">{isCR(obj) ? <ClusterServiceVersionResourceLink obj={obj} /> : <ResourceLink kind={obj.kind} name={obj.metadata.name} namespace={obj.metadata.namespace} title={obj.metadata.name} />}</div>
  ...
  return (
    <MultiListPage
      ...
      ListComponent={listProps => <List {...listProps} data={listProps.data.map(o => ({ ...o, rowKey: o.metadata.uid }))} EmptyMsg={() => <MsgBox title="No Resources Found" detail={`There are no Kubernetes resources used by this ${props.obj.kind}.`} />} Header={ResourceHeader} Row={ResourceRow} />}
    />
  );
});
