...
export class CardList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: props.data.map(item => ({ ...item, isActive: false })),
    };
  }
  onClickCard = index => {
    this.setState({
      data: this.state.data.map((item, idx) => (item.isActive === true ? { ...item, isActive: false } : idx === index ? { ...item, isActive: true } : item)),
    });
  };
  // active state 관리
  render() {
    ...
    return (
      <div className="cards-pf">
        <div className="container-fluid container-cards-pf">
          <div className="row row-cards-pf">
            {data.map((item, index) => (
              <ServiceClassCard key={index} index={index} isRecommended={item.isRecommended} isNew={item.isNew} isActive={item.isActive} onClickCard={this.onClickCard} />
            ))}
          </div>
        </div>
      </div>
