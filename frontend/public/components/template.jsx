...
const TemplateRow = kind => function TemplateRow({ obj }) {
    return <div className="row co-resource-list__item">
        <div className="col-xs-3 col-sm-3 co-resource-link-wrapper">
            <ResourceCog actions={menuActions} kind={referenceForModel(TemplateModel)} resource={obj} />
            <ResourceLink kind={referenceForModel(TemplateModel)} name={obj.metadata.name} namespace={obj.metadata.namespace} title={obj.metadata.name} />
        </div>
...
export const TemplatesPage = props =>
    <ListPage {...props} ListComponent={TemplateList} canCreate={true} kind={referenceForModel(TemplateModel)} />;
