...
const AWXTableRow: RowFunction<K8sResourceKind> = ({ obj: awx, index, key, style }) => {
  custommMenuActionUrl = `https://${awx.spec?.tower_hostname}`;
  return (
    <TableRow id={awx.metadata.uid} index={index} trKey={key} style={style}>
      ...
      <TableData className={tableColumnClasses[4]}>
        <ResourceKebab actions={menuActions} kind={kind} resource={awx} customData={{ label: customMenuActionLabel, url: custommMenuActionUrl }} />
      </TableData>
    </TableRow>
  );
};
...
export const AWXDetailsList: React.FC<AWXDetailsListProps> = ({ obj: awx }) => {
  ...
  custommMenuActionUrl = awx.spec?.tower_hostname ? `https://${awx.spec?.tower_hostname}` : undefined;
  return (
    <dl className="co-m-pane__details">
      ...
      <DetailsItem label={t('MULTI:MSG_MULTI_AWXINSTANCES_AWXINSTANCEDETAILS_2')} obj={awx} path="spec.tower_hostname">
        {awx.spec?.tower_hostname ? (<a href={custommMenuActionUrl} target="_blank" >{awx.spec.tower_hostname}</a>) : (<div>-</div>)}
      </DetailsItem>
...
export const AWXsDetailsPage: React.FC<DetailsPageProps> = (props) => <DetailsPage {...props} kind={kind} menuActions={menuActions} customData={{ label: customMenuActionLabel, url: custommMenuActionUrl }} getResourceStatus={awxStatusReducer} pages={[details(detailsPage(AWXDetails)), editYaml()]} />;;
