...
export const InviteMemberModal = withHandlePromise((props: InviteMemberModalProps) => {
  ...
  const submit: React.FormEventHandler<HTMLFormElement> = e => {
    e.preventDefault();
    const promise = coFetchJSON(`/api/multi-hypercloud/namespaces/${props.namespace}/clustermanagers/${props.clusterName}/member_invitation/${type}/${selectedMember}?userId=${getId()}${getUserGroup()}&remoteRole=${role}`, 'POST');
        ...
        <Section id="user">
          <div className="hc-invite-modal__input-members">
            ...
            <div className="hc-invite-modal__members">
              <TextInput type="text" name="selected-member" value={selectedMember} onChange={selectMember} />
            </div>
            <div>
              {type === 'user'
                ? t('MULTI:MSG_MULTI_CLUSTERS_INVITEPEOPLEPOPUP_SUBMESSAGE_1')
                    .split('\n')
                    .map(line => {
                      return (
                        <span>
                ...
                : t('MULTI:MSG_MULTI_CLUSTERS_INVITEPEOPLEPOPUP_SUBMESSAGE_2')
                    .split('\n')
                    .map(line => {
                      return (
                        <span>
