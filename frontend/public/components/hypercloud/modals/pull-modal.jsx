...
const BasePullModal = withTranslation()(
  class BasePullModal extends PromiseComponent {
    constructor(props) {
      super(props);
    ...
    _submit(e) {
      ...
      const { baseBranch, headBranch } = this.state;
      const namespace = resource?.metadata?.namespace;
      const name = resource?.metadata?.name;

      const url = `${document.location.origin}/api/kubernetes/apis/cicdapi.tmax.io/v1/namespaces/${namespace}/integrationconfigs/${name}/runpre`;
      //const url = `https://192.168.9.194:6443/apis/cicdapi.tmax.io/v1/namespaces/${namespace}/integrationconfigs/${name}/runpre`;      
      const body = {
        ...
        <form onSubmit={this._submit} name="form" className="modal-content">
          ...
          <ModalBody>
            <Section label={t('COMMON:MSG_MAIN_POPUP_24')} id="headBranch" isRequired={true} >
              <input className="pf-c-form-control" id="headBranch" name="headBranch" onChange={this.onChangeHeadBranch} value={this.state.branch} />
            </Section>
            <Section label={t('COMMON:MSG_MAIN_POPUP_25')} id="baseBranch" description={t('COMMON:MSG_MAIN_POPUP_26')} isRequired={true} >
              <input className="pf-c-form-control" id="baseBranch" name="baseBranch" onChange={this.onChangeBaseBranch} value={this.state.branch} />
            </Section>
          </ModalBody>
...
export const pullModal = createModalLauncher(props => <BasePullModal {...props} />);
