...
/**
 * Firehose로 리소스리스트를 불러올 수 있는 다중선택 드롭다운 컴포넌트이다. item 선택 시 드롭다운 하위에 chip의 형태로 선택된 항목이 나타난다.
 ...
 * @prop {any[]} defaultValues - 드롭다운의 기본 선택값을 지정해주는 속성. [{lable: 'AAA', value: 'aaa'}, {label: 'BBB', value: 'bbb'}] 형태로 설정해주고, 해당 item은 items props에 존재하는 item이어야 한다. (Controller로 감싸서 ListView컴포넌트 안에 사용 시 Controller의 defaultValue속성에도 같은 값을 지정해줘야 한다)
 */
export const MultiSelectDropdownFirehose = React.forwardRef<HTMLInputElement, MultiSelectDropdownFirehoseProps>((props, ref) => {
  return (
    <Firehose resources={props.resourcesConfig}>
      <MultiSelectDropdownWithRef {...props} useResourceItemsFormatter={true} ref={ref} />
    </Firehose>
  );
});
