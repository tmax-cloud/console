...
const Node = ({ className, id, children, description, valid }) => (
  <div className={className} key={id}>
    <div key={`${id}-child`}>{children}</div>
    <div className="row" />
...
const CombineNodes = (id, description, children, valid) => {
  ...
  return isArray ? children.map((cur, idx) => <Node className={className} id={`${id}-${idx}`} children={cur} description={description} valid={valid} />) : <Node id={id} className={className} children={children} description={description} valid={valid} />;
};
