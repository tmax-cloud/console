  ...
  return (
    <TableRow id={helmrelease.name} index={index} trKey={key} style={style}>
      <TableData className={tableColumnClasses[0]}>
        <ResourceLink kind={kind} name={helmrelease.name} namespace={helmrelease.namespace} title={helmrelease.name} />
      </TableData>
...
export const Helmreleases: React.FC = props => {
  const { t } = useTranslation();
  return <Table {...props} aria-label="Experiments" Header={HelmreleaseTableHeader.bind(null, t)} Row={HelmreleaseTableRow} virtualize />;
};