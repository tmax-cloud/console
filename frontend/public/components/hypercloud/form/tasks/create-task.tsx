...
const CreateTaskComponent: React.FC<TaskFormProps> = props => {
  ...
  //   e.preventDefault();
  //   let data = methods.getValues(); // modal에서 입력받은 data
  //   let currStep = { name: data.name, path: data.path, type: data.type, option: data.option };
  //   setStep(() => {
    ...
    <>
      ...
      <Section label="Input Resource" id="inputResource">
        <>
          ...
          <span className="open-modal_text" onClick={() => ModalLauncher({ inProgress: false, title: 'Input Resource', handleMethod: onAddInputResource, children: <InputResourceModal methods={methods} inputResource={inputResource} />, submitText: '추가' })}>
            + 인풋 리소스 추가
          </span>
        </>
      </Section>
      <Section label="Output Resource" id="outputResource">
        <>
          ...
          <span className="open-modal_text" onClick={() => ModalLauncher({ inProgress: false, title: 'Out Resource', handleMethod: onAddOutputResource, children: <OutputResourceModal methods={methods} outputResource={outputResource} />, submitText: '추가' })}>
            + 아웃풋 리소스 추가
          </span>
        </>
      </Section>
      <Section label="태스크 파라미터 구성" id="taskParamter">
        <>
          ...
          <span className="open-modal_text" onClick={() => ModalLauncher({ inProgress: false, title: '태스크 파라미터', handleMethod: onAddTaskParameter, children: <TaskParameterModal methods={methods} taskParameter={taskParameter} />, submitText: '추가' })}>
            + 태스크 파라미터 추가
          </span>
        </>
      </Section>
      <Section label="워크스페이스 구성" id="workSpace">
        <>
          ...
          <span className="open-modal_text" onClick={() => ModalLauncher({ inProgress: false, title: '워크스페이스', handleMethod: onAddWorkSpace, children: <WorkSpaceModal methods={methods} workSpace={workSpace} />, submitText: '추가' })}>
            + 워크스페이스 추가
          </span>
        </>
      </Section>
      <Section label="볼륨" id="volume">
        <>
          ...
          <span className="open-modal_text" onClick={() => ModalLauncher({ inProgress: false, title: '볼륨', handleMethod: onAddVolume, children: <VolumeModal methods={methods} volume={volume} />, submitText: '추가' })}>
            + 볼륨 추가
          </span>
        </>
      </Section>
      {/* <Section label="스텝" id="step">
        <>
          <ModalList list={step} id="step" title="스텝 구성" children={<StepModal methods={methods} />} onRemove={onRemoveStep} onModify={onModifyStep} methods={methods} description="이 태스크와 연결된 스텝이 없습니다."></ModalList>
          <span className="open-modal_text" onClick={() => ModalLauncher({ inProgress: false, title: '스텝', handleMethod: onAddStep, children: <StepModal methods={methods} />, submitText: '추가' })}>
            + 스텝 추가
          </span>
        </>
      </Section> */}
