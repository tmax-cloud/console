...
export const NonK8SListPage: React.FC<NonK8SListPageProps> = props => {
    ...
    const nameFilterFunction = (data: any, name: string) => {
        if (!name || name === '') { return true; }
            ...
            if (fuzzy(toLower(name), toLower(data.name))) { return true; }
            else { return false; }
        }
    };
    const rowFilterFunction = (data: any, rowFilter: string) => {
        const filterList = rowFilter?.split(',');
        if (!filterList || filterList?.length === 0 || filterList[0] === '') { return true; }
        else {
            if (filterList.indexOf(reducer(data)) > -1) { return true; }
            else { return false; }
        }
    };
