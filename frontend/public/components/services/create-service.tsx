...
const Requestform = SubForm =>
  class ServiceFormComponent extends React.Component<BaseEditServiceProps_, BaseEditServiceState_> {
    constructor(props) {
      super(props);
          ...
          <form className="co-m-pane__body-group co-create-service-form" onSubmit={this.save}>
            ...
            <fieldset disabled={!this.props.isCreate}>
              ...
              <div className="form-group">
                ...
                <AdvancedPortEditor portPairs={this.state.ports} updateParentData={this._updatePorts} t={t} />
              </div>
              <div className="form-group">
                <label className="control-label" htmlFor="service-type">
                  {t('CONTENT:TYPE')}
                </label>
                <div>
              ...
              <ButtonBar errorMessage={this.state.error} inProgress={this.state.inProgress}>
                ...
                <Link to={formatNamespacedRouteForResource('services')} className="btn btn-default" id="cancel">
                  {t('CONTENT:CANCEL')}
                </Link>
              </ButtonBar>
            </fieldset>
...
const SecretLoadingWrapper = props => {
  ...
  return (
    <StatusBox {...props.obj}>
      <ServiceFormComponent {...props} t={t} serviceTypeAbstraction={serviceTypeAbstraction} obj={props.obj.data} fixed={fixed} explanation={pageExplanation[serviceTypeAbstraction]} />
    </StatusBox>
  );
};
...
export const CreateService = ({ match: { params } }) => {
  ...
  const { t } = useTranslation();
  return <ServiceFormComponent fixed={{ metadata: { namespace: params.ns } }} t={t} serviceTypeAbstraction={params.type} explanation={pageExplanation[params.type]} titleVerb="Create" isCreate={true} />;
};
