          ...
          <div className="row">
            ...
            <div className="col-sm-6 col-xs-12">
              <dl className="co-m-pane__details">
                ...
                <dt>Replicas</dt>
                <dd>
                  {this.state.desiredCountOutdated ? (
                    <LoadingInline />
                  ) : (
                    <a className="co-m-modal-link" href="#" onClick={this._openReplicaCountModal}>
                ...
                <dd>
                  <span className="text-muted">Memory:</span> {_.get(spec, 'resources.requests.memory')}
                </dd>
              </dl>
            </div>
          </div>
...
export const AlertManagersDetailsPage = props => <DetailsPage {...props} pages={[details(Details), editYaml()]} />;
          ...
          {!props.loaded ? (
            ...
            _.map(props.alertmanagers.data, (alertManager, i) => (
              <div className="alert-manager-row" key={i}>
                <ResourceLink kind="Alertmanager" name={alertManager.metadata.name} namespace={alertManager.metadata.namespace} title={alertManager.metadata.uid} />
              </div>
            ))
          )}
        </div>
...
const AlertManagerRow = ({ obj: alertManager }) => {
  ...
  return (
    <ResourceRow obj={alertManager}>
      <div className="col-md-2 col-sm-3 col-xs-6">
        <ResourceLink kind="Alertmanager" name={metadata.name} namespace={metadata.namespace} title={metadata.name} />
      </div>
      <div className="col-md-2 col-sm-3 col-xs-6">
...
const AlertManagerHeader = props => (
  <ListHeader>
    <ColHead {...props} className="col-md-2 col-sm-3 col-xs-6" sortField="metadata.name">
      Name
    </ColHead>
    <ColHead {...props} className="col-md-2 col-sm-3 col-xs-6" sortField="metadata.namespace">
      Namespace
    </ColHead>
    <ColHead {...props} className="col-md-4 col-sm-3 hidden-xs" sortField="metadata.labels">
      Labels
    </ColHead>
    <ColHead {...props} className="col-md-1 hidden-sm hidden-xs" sortField="spec.version">
      Version
    </ColHead>
...
export const AlertManagersPage = props => <ListPage {...props} ListComponent={AlertManagersList} canCreate={false} kind="Alertmanager" />;
