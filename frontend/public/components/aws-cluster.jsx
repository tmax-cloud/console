    ...
    return (
      <React.Fragment>
        <div className="co-m-pane__body">
          <SectionHeading text={t('ADDITIONAL:OVERVIEWTITLE', { something: ResourcePlural('AWSCluster', t) })} />
...
export const AWSClusterList = props => {
  const { kinds } = props;
  const Row = AWSClusterRow(kinds[0]);
  Row.displayName = 'AWSClusterRow';
  return <List {...props} Header={AWSClusterHeader} Row={Row} />;
};
AWSClusterList.displayName = AWSClusterList;
...
export const AWSClustersDetailsPage = props => {
  const { t } = useTranslation();
  ...
  return <DetailsPage {...props} kind="AWSCluster" menuActions={menu} pages={page} />;
};

AWSClustersDetailsPage.displayName = 'AWSClustersDetailsPage';
