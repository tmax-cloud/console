...
export const ResourceLink = connectToModel(({ className, kind, name, namespace, title, displayName, linkTo = true, kindsInFlight }) => {
  if (kindsInFlight) {
    return null;
  }

  let path = resourcePath(kind, name, namespace);
  const value = displayName ? displayName : name;

  console.log(path, kind, name, namespace);
  return (
    <span className={classNames('co-resource-link', className)}>
      <ResourceIcon kind={kind} />
      {linkTo && path ? (
        <Link to={path} title={title} className="co-resource-link__resource-name">
          {value}
        </Link>
      ) : (
        <span className="co-resource-link__resource-name">{value}</span>
      )}
    </span>
  );
});
...
const NodeLink_ = props => {
  ...
  return flags[FLAGS.CAN_LIST_NODE] ? <ResourceLink kind="Node" name={name} title={name} /> : <React.Fragment>{name}</React.Fragment>;
};
