...
class TagsModal extends PromiseComponent {
  ...
  _submit(e) {
    ...
    if (_.uniq(keys).length !== keys.length) {
      this.setState({ errorMessage: this.props.t('VAILDATION:DUPLICATE-KEY') });
      return;
    }
  ...
  render() {
    ...
    return (
      <form onSubmit={this._submit}>
        <ModalTitle>{this.props.t('ADDITIONAL:EDIT', { something: this.props.t('CONTENT:ANNOTATIONS') })}</ModalTitle>
        <ModalBody>
          <NameValueEditorComponent nameValuePairs={tags} submit={this._submit} updateParentData={this._updateTags} t={this.props.t} />
        </ModalBody>
        <ModalSubmitFooter submitText={this.props.t('CONTENT:SAVECHANGES')} cancel={this._cancel} errorMessage={this.state.errorMessage} inProgress={this.state.inProgress} />
      </form>
    );
  }
}

export const annotationsModal = createModalLauncher(props => <TagsModal path="/metadata/annotations" tags={props.resource.metadata.annotations} title="Edit Annotations" {...props} />);
