...
//Modal for resource deletion and allows cascading updates if propagationPolicy is provided for the enum
class UpdateModal extends PromiseComponent {
  constructor(props) {
    super(props);
  ...
  _submit(event) {
    ...
    if (nonk8sProps) {
      const url = this.props.nonk8sProps.updateServiceURL;
      this.handlePromise(coFetchJSON.update(url)).then(() => {
        this.props.close();
      ...
      const json = propagationPolicy ? { kind: 'UpdateOptions', apiVersion: 'v1', propagationPolicy } : null;

      this.handlePromise(k8sKill(kind, resource, {}, json)).then(() => {
        this.props.close();

        // If we are currently on the updated resource's page, redirect to the resource list page
...
export const updateModal = createModalLauncher(withTranslation()(UpdateModal));
