...
export const AddSecretToWorkloadModal = withTranslation()(
  class AddSecretToWorkloadModal extends React.Component<any, any> {
    state = {
      inProgress: false,
      errorMessage: '',
      workloadOptions: {},
    ...
    render() {
      ...
      const selectWorkloadPlaceholder = t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_5');

      return (
        <form onSubmit={this.submit} name="co-add-secret-to-workload" className="co-add-secret-to-workload modal-content">
          <ModalTitle>{t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_1')}</ModalTitle>
          <ModalBody>
            <p>
              {t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_2')}
              <ResourceIcon kind="Secret" />
              {secretName} {t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_3')}
            </p>
            <div className="form-group">
              <label className="control-label co-required" htmlFor="co-add-secret-to-workload__workload">
                {t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_4')}
              </label>
            ...
            <fieldset>
              <legend className="co-legend co-required">{t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_6')}</legend>
              <RadioInput title={t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_7')} name="co-add-secret-to-workload__add-as" id="co-add-secret-to-workload__envvars" value="environment" onChange={this.onAddAsChange} checked={addAsEnvironment} />
              {addAsEnvironment && (
                <div className="co-m-radio-desc">
                  <div className="form-group">
                    <label htmlFor="co-add-secret-to-workload__prefix">{t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_8')}</label>
                    <input className="pf-c-form-control" name="prefix" id="co-add-secret-to-workload__prefix" placeholder={t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_9')} type="text" onChange={this.handleChange} />
                  </div>
                </div>
              )}
              <RadioInput title={t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_10')} name="co-add-secret-to-workload__add-as" id="co-add-secret-to-workload__volume" value="volume" onChange={this.onAddAsChange} checked={addAsVolume} />
              {addAsVolume && (
                <div className="co-m-radio-desc">
                  <div className="form-group">
                    <label htmlFor="co-add-secret-to-workload__mountpath" className="co-required">
                      {t('SINGLE:MSG_SECRETS_SECRETDETAILS_ADDSECRETTOWORLOAD_11')}
                    </label>
          ...
          <ModalSubmitFooter errorMessage={this.state.errorMessage} inProgress={this.state.inProgress} submitText={t('COMMON:MSG_COMMON_BUTTON_COMMIT_3')} cancel={this.props.cancel} />
        </form>
      );
    }
  },
...
export const configureAddSecretToWorkloadModal = createModalLauncher<any>(AddSecretToWorkloadModal);
