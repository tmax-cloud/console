...
export const getQueryKey = (query: string, timespan?: number): string => (timespan ? `${query}@${timespan}` : query);

export const getPrometheusQueryResponse = (prometheusResults: RequestMap<PrometheusResponse>, query: string, timespan?: number): [PrometheusResponse, any] => {
...
const fetchPeriodically: FetchPeriodically = async (dispatch, type, key, getURL, getState, fetch) => {
...
export const watchPrometheusQuery: WatchPrometheusQueryAction = (query, namespace, timespan) => (dispatch, getState) => {
  const queryKey = getQueryKey(query, timespan);
  ...
  if (!isActive) {
    const prometheusBaseURL = namespace ? window.SERVER_FLAGS.prometheusTenancyBaseURL : window.SERVER_FLAGS.prometheusBaseURL;
...
export const stopWatchPrometheusQuery: StopWatchPrometheusAction = (query, timespan) => stopWatch(RESULTS_TYPE.PROMETHEUS, getQueryKey(query, timespan));
export const stopWatchURL = (url: string) => stopWatch(RESULTS_TYPE.URL, url);
export const clearPrometheusData: ClearPrometheusDataAction = (query, timespan) => setData(RESULTS_TYPE.PROMETHEUS, getQueryKey(query, timespan), undefined);

type ThunkAction = (dispatch: Dispatch, getState: () => RootState) => void;

export type WatchURLAction = (url: string, fetch?: Fetch) => ThunkAction;
export type WatchPrometheusQueryAction = (query: string, namespace?: string, timespan?: number) => ThunkAction;
export type StopWatchURLAction = (url: string) => void;
export type StopWatchPrometheusAction = (query: string, timespan?: number) => void;
export type ClearPrometheusDataAction = (query: string, timespan?: number) => void;

export type Fetch = (url: string) => Promise<any>;

type FetchPeriodically = (dispatch: Dispatch, type: RESULTS_TYPE, key: string, getURL: () => string, getState: () => RootState, fetch: Fetch) => void;
