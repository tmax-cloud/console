...
export const nonK8sloaded = (id: string, k8sObjects: K8sResourceKind | K8sResourceKind[]) => action(ActionType.NonK8sLoaded, { id, k8sObjects });
...
export const watchNonK8sObject = (id: string, name: string, namespace: string, query: { [key: string]: string }, k8sType: K8sKind) => (dispatch: Dispatch, getState) => {
  if (id in REF_COUNTS) {
    REF_COUNTS[id] += 1;
    return nop;
  }
...
export const watchNonK8sList = (
  ...
  const incrementallyLoad = async (continueToken = ''): Promise<string> => {
    ...
    if (!continueToken) {
      [nonK8sloaded, extraAction].forEach(f => f && dispatch(f(id, result)));
    } else {
      dispatch(bulkAddToList(id, result));
    }
