...
export type ConnectToPlural = <P extends WithPluralProps>(C: React.ComponentType<P>) => React.ComponentType<Omit<P, keyof WithPluralProps>> & { WrappedComponent: React.ComponentType<P> };
...
export const connectToPlural: ConnectToPlural = connect((state: State, props: { plural?: GroupVersionKind | string; match: match<{ plural: GroupVersionKind | string }> }) => {
  ...
  const kindObj = isGroupVersionKind(plural) ? state.k8s.getIn(['RESOURCES', 'models']).get(plural) : _.find(k8sModels, model => model.plural === plural);

  const modelRef = !resourceType ? (isGroupVersionKind(plural) ? plural : _.get(kindObj, 'kind')) : resourceType; // resourceType (federatedresource메뉴에서 resource kind에 따라 details 페이지 url path 분기 로직)
