...
export const referenceForGroupVersionKind = (group: string) => (version: string) => (kind: string) => {
  if (kind === 'Task' || kind === 'ClusterTask' || kind === 'TaskRun' || kind === 'Pipeline' || kind === 'PipelineRun' || kind === 'PipelineResource' || kind === 'ServiceClass' || kind === 'ClusterServiceClass'|| kind === 'Template' || kind === 'ClusterTemplate') {
    return kind;
  }
